...
phi = 0, theta = 0;
var controlConfig = { Refraction: dRefract, 'Sprite dist.': fovFac, 'Diamond hue': dHue, "Diamond glow": dGlow, 'Hue to glow': true };
var diamondWatcher;
init();
animate();
...
function setRR(rR, upperLimit) {
    ...
    var retV3 = new THREE.Vector3(); retV3.x = (baseR);
...
function setHG(dHueColor, dGlow) {
    if (controlConfig.valueOf('Hue to glow'))
...
function init() {
    ...
    guiControlContainer.add(controlConfig, "Refraction", 1.0, 2.4).onChange(function (v) {uniforms2["mRefractionRatio"].value = setRR(v, 1.35); uniforms1["mRefractionRatio"].value = setRR(v, 1.15);});
    ...
    guiControlContainer.add(controlConfig, 'Hue to glow', true).onChange(function () { uniforms1["mHueGlow"].value = setHG(dHueColor, dGlow); uniforms2["mHueGlow"].value = setHG(dHueColor, dGlow); });
