...
var touch_prev_dist, info, dinInfo;
...
var dcDist, dlx, dly, dlz, fovFac = 0.5, distToCamera;
...
phi = 0, theta = 0;
var controlConfig = { refraction: 2.4, spriteDist: fovFac };
var diamondWatcher;
init();
animate();
...
function init() {
    ...
    */
    ...
    dinInfo = document.createElement('div');
    info.appendChild(dinInfo);
   ...
   uniforms1["mFresnelBias"].value = 0.1;
    ...
    guiControlContainer.add(controlConfig, "refraction", 1.0, 2.4).onChange(function (v) {
        uniforms2["mRefractionRatio"].value = 1.0 + 0.35 / 1.4 * (v - 1); uniforms1["mRefractionRatio"].value = 1.0 + 0.15 / 1.4 * (v - 1);
        dinInfo.innerText = 'RR1: ' + uniforms1["mRefractionRatio"].value + 'RR2: ' + uniforms2["mRefractionRatio"].value;

    });
    guiControlContainer.add(controlConfig, "spriteDist", 0.0, 1.0).onChange(function (v) {fovFac = v;});
